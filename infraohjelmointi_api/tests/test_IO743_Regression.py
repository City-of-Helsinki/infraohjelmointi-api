...
class IO743RegressionTestCase(TestCase):
    ...
    # TSE-2028 scenario constants from IO-743 original bug report
    TSE_2028_PARENT_BUDGET = 58000
    TSE_2028_CHILD1_BUDGET = 30000  
    TSE_2028_CHILD2_BUDGET = 28000  # Total: 30k + 28k = 58k (equal to parent)
    ...
    def setUp(self):
        ...
        # Create the exact TSE-2028 budget scenario
        self.create_financial_scenario(
            self.tse_2028, self.TSE_2028_PARENT_BUDGET, 
            self.child_class_1, self.TSE_2028_CHILD1_BUDGET, 
            self.child_class_2, self.TSE_2028_CHILD2_BUDGET
        )
    ...
    def test_tse_2028_scenario_new_method(self):
        ...
        frame_budgets[f"{self.year}-{self.tse_2028.id}"] = self.TSE_2028_PARENT_BUDGET
        frame_budgets[f"{self.year}-{self.child_class_1.id}"] = self.TSE_2028_CHILD1_BUDGET
        frame_budgets[f"{self.year}-{self.child_class_2.id}"] = self.TSE_2028_CHILD2_BUDGET
    ...
    def test_actual_overlap_scenario_both_methods(self):
        ...
        # Parent budget is LESS than children sum (overlap scenario)
        overlap_parent_budget = 50000  # 50k < 58k (30k + 28k)
        self.create_financial_scenario(
            overlap_parent, overlap_parent_budget, 
            overlap_child_1, self.TSE_2028_CHILD1_BUDGET, 
            overlap_child_2, self.TSE_2028_CHILD2_BUDGET
        )
        ...
        # Test new method
        frame_budgets = defaultdict(lambda: 0)
        frame_budgets[f"{self.year}-{overlap_parent.id}"] = overlap_parent_budget
        frame_budgets[f"{self.year}-{overlap_child_1.id}"] = self.TSE_2028_CHILD1_BUDGET
        frame_budgets[f"{self.year}-{overlap_child_2.id}"] = self.TSE_2028_CHILD2_BUDGET

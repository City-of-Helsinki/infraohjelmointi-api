...
class TaskTestCase(TestCase):
    task_1_Id = uuid.UUID("bbba45f2-b0d4-4297-b0e2-4e60f8fa8412")
    task_2_Id = uuid.UUID("be923535-0b96-4cb5-b357-5e62a145281f")
    personId = uuid.UUID("f2f17b71-2d9a-4ddc-ba88-948a172c7bde")
    ...
    def setUpTestData(self):
        self.taskStatus = DataGen.mkTaskStatus(id=self.task_1_Id)
    ...
    def test_Task_is_created(self):
        ...
        self.assertEqual(
            Task.objects.filter(id=self.task_1_Id).exists(),
            True,
            msg="Created Task with Id {} does not exist in DB".format(self.task_1_Id),
        )
        task = Task.objects.get(id=self.task_1_Id)
    ...
    def test_foreign_keys_exist(self):
        self.assertDictEqual(
            ...
            Task.objects.filter(id=self.task_1_Id).values()[0],
            msg="Project foreign key does not exist in Task with id {}".format(
                self.task_1_Id
            ),
        )
        self.assertDictEqual(
            self.person.task_set.all().values()[0],
            Task.objects.filter(id=self.task_1_Id).values()[0],
    ...
    def test_GET_all_Tasks(self):
        ...
        DataGen.mkTask(id=self.task_2_Id, projectId=self.project)
        response = self.client.get("/tasks/")
        self.assertEqual(response.status_code, 200, msg="Status Code != 200")
        self.assertEqual(len(response.json()), 2, msg="Number of returned Tasks != 2")

    def test_GET_one_Task(self):
        response = self.client.get("/tasks/{}/".format(self.task_1_Id))
        self.assertEqual(
        ...
        serializer = TaskSerializer(Task.objects.get(id=self.task_1_Id), many=False)
    ...
    def test_DELETE_Task(self):
        response = self.client.delete("/tasks/{}/".format(self.task_1_Id))
        self.assertEqual(
            response.status_code,
            204,
            msg="Error deleting task with Id {}".format(self.task_1_Id),
        )
        self.assertEqual(
            Task.objects.filter(id=self.task_1_Id).exists(),
            False,
            msg="Task with Id {} still exists in DB".format(self.task_1_Id),
        )
